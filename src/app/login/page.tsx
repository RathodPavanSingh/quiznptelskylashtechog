...
import { User as UserIcon, Lock, Loader2, LogIn, AlertCircle, Eye, EyeOff } from "lucide-react";

const inputCls =
  "w-full rounded-xl border border-slate-200 bg-slate-50 py-3 pl-11 pr-11 text-sm text-slate-900 placeholder:text-slate-400 outline-none transition focus:border-orange-400 focus:bg-white focus:ring-4 focus:ring-orange-100";
  ...
  return (
    <div className="rounded-3xl border border-slate-200 bg-white p-7 shadow-2xl shadow-orange-950/20 sm:p-8">
      ...
      <p className="mt-2 text-sm leading-relaxed text-slate-500">
        Sign in with your username, email, Google or Apple ID. Don&apos;t have account?{" "}
        <Link href={next ? `/signup?next=${encodeURIComponent(next)}` : "/signup"} className="font-bold text-orange-600 hover:underline">
          Sign Up
        </Link>
      </p>
      ...
      {next && (
        <div className="mt-5 rounded-xl border border-blue-200 bg-blue-50 px-4 py-2.5 text-xs font-semibold text-blue-700">
          Sign in required to continue to <span className="font-mono">{next}</span>
        </div>
      )}
      ...
      <form className="mt-6 space-y-3.5" onSubmit={submit}>
        <div className="relative">
          ...
          <input value={identifier} onChange={(e) => setIdentifier(e.target.value)} placeholder="Username or Email" required className={inputCls} autoComplete="username" />
        </div>
        <div className="relative">
          ...
          />
          <button type="button" onClick={() => setShowPw((v) => !v)} aria-label={showPw ? "Hide password" : "Show password"} className="absolute right-3.5 top-1/2 -translate-y-1/2 text-slate-400 transition hover:text-slate-700">
            {showPw ? <EyeOff className="h-4 w-4" /> : <Eye className="h-4 w-4" />}
          </button>
        </div>

        <div className="text-right">
          <button type="button" onClick={() => setNote(true)} className="text-xs font-bold text-orange-600 hover:underline">
            Forget Password?
          </button>
        </div>
        {note && (
          <p className="rounded-xl border border-slate-200 bg-slate-50 px-4 py-2.5 text-[11px] leading-relaxed text-slate-500">
            Password reset email isn&apos;t wired in this environment — sign in with Google/Apple instead, or ask your admin.
          </p>
        )}
        ...
        >
          {loading ? <Loader2 className="h-4 w-4 animate-spin" /> : <LogIn className="h-4 w-4 transition group-hover:scale-110" />}
